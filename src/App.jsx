...
function SplashIntro({ onFinish, lang }) {
  ...
  useEffect(() => {
    ...
    const t2 = setTimeout(() => onFinish(), 4300);
  ...
  return (
    <div className={"mt-intro" + (exiting ? " exiting" : "")} onClick={skip}>
      <div className="mt-intro-inner">
        <svg viewBox="0 0 400 260" className="mt-intro-svg">
          <defs>
          ...
          {pins.map((p, i) => (
            <g key={i} className="mt-intro-pin" style={{ animationDelay: `${1.9 + i * 0.28}s` }}>
              <circle cx={p.x} cy={p.y} r="9" />
              <text x={p.x} y={p.y + 3.5} textAnchor="middle">{p.n}</text>
            </g>
          ))}
        </svg>
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .mt-intro-inner { display:flex; flex-direction:column; align-items:center; gap:16px; }
        .mt-intro-svg { width:min(88vw,420px); height:auto; overflow:visible; }
        .mt-intro-land { fill:#DCEEDF; stroke:#B9D9C4; stroke-width:1.5; }
        .mt-intro-flightpath { fill:none; stroke:#256D64; stroke-width:1.3; stroke-dasharray:3 4; opacity:.45; }
